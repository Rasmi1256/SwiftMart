...
// @desc    Get batch stock status for multiple products
// @route   POST /api/inventory/batch
// @access  Public
export const getBatchStockStatus = async (req: Request, res: Response) => {
  try {
    ...
    // For products without inventory items, return default values
    ...
    const result = Array.from(stockMap.entries()).map(([productId, status]) => ({
      productId,
    ...
    console.error('Error fetching batch stock status:', error);
    res.status(500).json({ message: 'Server error fetching batch stock status.' });
  }
};
