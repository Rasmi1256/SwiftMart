...
import  { prisma,Order, OrderItem, Prisma } from 'database';
...
// @desc    Get pending orders for batching (route optimization)
// @route   GET /api/orders/pending/batch
// @access  Private (Admin)
export const getPendingOrdersForBatching = async (req: Request, res: Response) => {
  try {
    const orders = await prisma.order.findMany({
      where: { status: 'placed' },
      include: { items: true },
    ...
    console.error('Error fetching pending orders for batching:', error);
    res.status(500).json({ message: 'Server error fetching pending orders.' });
  }
};

// @desc    Batch orders for route optimization
// @route   POST /api/orders/batch/route
// @access  Private (Admin)
export const batchOrdersForRoute = async (req: Request, res: Response) => {
  ...
  try {
    const orders = await prisma.order.findMany({
      where: { id: { in: orderIds }, status: 'placed' },
      include: { items: true },
    });

    if (orders.length !== orderIds.length) {
      return res.status(404).json({ message: 'Some orders not found or not in placed status.' });
    }

    // Send to route optimization service
    const response = await axios.post(`${config.routeOptimizationServiceUrl}/optimize`, {
    ...
    res.status(200).json({
      message: 'Orders batched for route optimization successfully',
      optimizedRoutes: response.data,
    });
  } catch (error: any) {
    console.error('Error batching orders for route:', error.response?.data || error);
    res.status(error.response?.status || 500).json({
      message: error.response?.data?.message || 'Failed to batch orders for route optimization.'
    });
  }
};
