# ai-ml/recommendation-service/src/services/product_fetcher.py
import requests
...
class ProductFetcher:
    ...
    def fetch_all_products(self, limit: int = 100, page: int = 1):
        """Fetches a list of products from the product catalog."""
        try:
            response = requests.get(f"{self.base_url}/products", params={"limit": limit, "page": page})
            response.raise_for_status()  # Raise an exception for HTTP errors (4xx or 5xx)
            data = response.json()
            return data.get('products', [])
        except requests.exceptions.RequestException as e:
            print(f"Error fetching products from Product Catalog Service: {e}", flush=True)
            return []
